...
const ADMIN_PASS = 'lovekraft2026';
...
export default function Admin() {
  ...
  const login = () => {
    if (pass === ADMIN_PASS) {
      setAuthed(true);
      setOrders(JSON.parse(localStorage.getItem('lovekraft_orders') || '[]'));
    } else {
  ...
  const updateStatus = (idx, status) => {
    ...
    setOrders(updated);
    localStorage.setItem('lovekraft_orders', JSON.stringify(updated));
  };
  ...
  if (!authed) return (
    <div className="admin-login">
      <div className="admin-login__card card">
        ...
        <h2>Admin Panel</h2>
        <p>LoveKraft Shop Owner Access</p>
